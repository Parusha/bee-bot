...
const runlogoutTest = async (formData, io) => {
  ...
  try {
    ...
    // Handle logout based on device type
    if (device === 'desktop') {
      io.emit('log', 'Waiting for logout button...');
      await page.waitForSelector('button[data-testid="logout-btn"]', { timeout: 5000 });

      io.emit('log', 'logout button found, clicking the button...');
      await page.click('button[data-testid="logout-btn"]');
    } else {
      io.emit('log', 'Waiting for logout button...');
      await page.waitForSelector('div[role="presentation"]', { timeout: 5000 });

      io.emit('log', 'Finding and clicking the logout button...');
